...
In GitHub Actions CI run 34522958965, `hum/internal/cli` dominated the race suite at 103.627s on Linux and 121.699s on macOS; its normal run took 52.756s and 62.908s respectively. A local uncached profile found 208 top-level tests and no `t.Parallel()` calls. Large contributors included `TestAttachedRunInterruptLifecycle` (6.78s), `TestEnsureDaemonWaitsForSequentialRecovery` (6.08s), `TestLogsSince` (4.98s), `TestLogsFollow` (4.78s), and `TestUpHumanProgress` (3.62s).
...
Scope: CLI test orchestration, synchronization, fixtures, and test-only helpers. Measure base and candidate revisions with identical commands on the same machine or runner and record all raw wall times plus medians in Implementation Notes.
