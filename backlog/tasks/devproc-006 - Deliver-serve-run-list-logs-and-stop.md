...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 `go test ./internal/cli -run 'TestForegroundServe|TestDaemonUnavailable'` exits 0 and covers foreground diagnostics on stderr, Ctrl+C on serve stopping managed groups before exit, and the exact `Start it with hum serve --daemon.` failure for every command when no socket answers.
...
- [ ] #5 With `HUM_RUNTIME_DIR` set to a temporary directory and a foreground `serve` running, the built binary performs attached and detached `run`, `list`, bounded and followed `logs`, `stop`, and both shutdown modes; every command exits as documented and all JSON/NDJSON decodes with stable fields.
