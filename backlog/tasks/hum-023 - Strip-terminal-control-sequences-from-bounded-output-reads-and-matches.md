...
Outcome: bounded output reads and every child-output pattern match operate on text with terminal control sequences removed, so agents and humans reading `hum logs`, `hum wait --match`, MCP `logs`, and readiness `match` get clean lines from tty sessions (HUM-022) and from ordinary processes that emit colour under `FORCE_COLOR` or similar. Stored bytes, cursors, and follow/attached-run rendering stay raw. This is a stateless, per-entry strip. It is deliberately not redraw collapsing or terminal emulation, and it makes no byte-fidelity promise over JSON.
...
Scope, where it applies: (1) after the daemon bounded `output` operation completes its ring read, it strips copies of stdout and stderr entry text before wire conversion; CLI `logs`, `logs --json`, `logs --tail`, and MCP `logs` therefore receive stripped text; (2) the ring read predicate and readiness tracker test stdout and stderr patterns against stripped text, which covers bounded reads, `logs --follow --match`, `wait --match`, and readiness `match`; (3) system-entry text and matching remain raw; (4) nothing else changes. Ring reads themselves continue returning raw entries, allowing every initial replay and later event from `logs --follow`, plus attached `run`, to carry raw bytes for terminal rendering. The ring stores only raw bytes. Retention, eviction, and cursors are unchanged; `MaxBytes` and entry-too-large decisions use raw stored lengths; `MaxEntries` and `Tail` count selected stored entries even though their child-stream match predicate uses stripped text. A bounded response retains an entry even when stripping makes its text empty, preserving its stream, time, cursor, and result metadata. No `--raw` flag or per-request option is added.

Semantics to document: a pattern written against raw escape bytes stops matching; patterns anchored at line start now match colourised output whose first byte was ESC. `logs --follow --match` selects using stripped child text but emits the selected raw entry. Because stripping is per entry, a sequence split across entries by the idle flush leaves its tail visible at the start of the next entry, and carriage-return redraw frames remain separate segments; both are stated limitations, not bugs. docs/design.md changes "raw text" in the entry description to "raw stored text, stripped on bounded read and match", and docs/coding-agents.md plus both skills drop the HUM-022 guidance that bounded logs are not terminal-sanitized.

Non-goals: carriage-return, backspace, or cursor-movement redraw collapsing; alternate-screen or any screen-state tracking; terminal emulation; changing stored bytes or cursors; changing follow or attached `run` rendering; a `--raw` or opt-out flag; byte-exact invalid-UTF-8 over JSON; stripping C1 8-bit controls; stripping BEL or other C0 controls outside escape sequences; stripping or sanitized matching of system entries; protocol shape or version changes.
