...
- [ ] #4 With `DEVPROC_RUNTIME_DIR` set to a temporary directory and no daemon running, the built binary runs `devproc run demo --detach -- sleep 30` successfully, `devproc serve --daemon` twice reports the same PID, the daemon survives closing the launching terminal, and `daemon.log` exists while the caller's stdout/stderr received nothing from the daemon.
<!-- AC:END -->
