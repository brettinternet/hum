...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 `go test ./internal/config` exits 0 and covers flag/environment/default precedence for every setting plus rejection of non-positive byte limits, limits below the 64 KiB minimum, and invalid or negative durations.
- [ ] #2 `go test ./internal/config -run TestRuntimeDir` exits 0 and proves the `DEVPROC_RUNTIME_DIR` override, XDG selection, and per-user temporary fallback on supported Unix platforms.
