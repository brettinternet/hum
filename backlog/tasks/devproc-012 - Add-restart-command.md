...
- [ ] #3 Against the integration fixture, `devproc restart api --json` returns a new PID with restarts incremented, a follower started before the restart prints the marker followed by the first line of the new process, and `devproc logs api --after-cursor <pre-restart cursor>` returns output spanning both incarnations.
<!-- AC:END -->
