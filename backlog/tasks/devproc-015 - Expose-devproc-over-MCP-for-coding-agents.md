---
id: DEVPROC-015
title: Expose hum over MCP for coding agents
status: To Do
assignee: []
...
Outcome: coding agents that speak MCP can list, inspect, read, wait on, restart, and stop hum processes through typed tools that delegate to the existing application services.

Scope: a stdio MCP server command (`hum mcp`) exposing list, status, logs, wait, restart, and stop tools with JSON schemas mirroring the CLI JSON output and bounds; transport translation, schemas, and MCP-specific error mapping only; project scoping from the server's cwd; documentation for registering the server with common coding agents. Update docs/design.md and README.md.
...
- [ ] #2 With a daemon and fixture process running, an MCP client session over stdio lists the tools, calls logs with a cursor, and receives bounded output identical to `hum logs --json`.
