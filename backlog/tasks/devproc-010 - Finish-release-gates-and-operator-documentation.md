...
- [ ] #5 `docs/design.md` names the future versioned manifest, shell-only skill, and stdio/Streamable HTTP MCP transports; it maps manifest-only start/up plus list/status/logs/wait/restart/stop through the existing daemon protocol client without defining a second supervisor.
