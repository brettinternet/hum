...
- [ ] #3 After fixture exit, human and JSON status report the terminal state and exit status; unknown names fail with a typed error, while an unavailable daemon fails without starting one and prints `Nothing is running. Start a process with hum run <name> -- <command>.`
<!-- AC:END -->
