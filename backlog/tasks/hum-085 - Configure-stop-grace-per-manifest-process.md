...
- [x] #6 mise exec go -- go test -race -count=100 -shuffle=1789133877229139000 -run '^TestProcessStopGraceOperations/restart_remove_replacement_and_shutdown_use_record_values$' ./internal/app exits 0 without hanging, and failure cleanup cannot block on an unfired synthetic grace timer
<!-- AC:END -->
...
AC#6 PASS — mise exec go -- go test -race -count=100 -shuffle=1789133877229139000 -run '^TestProcessStopGraceOperations/restart_remove_replacement_and_shutdown_use_record_values$' ./internal/app exited 0. Replaced the synthetic timer's one-second polling deadline with condition-variable synchronization, added explicit signal observation for concurrently stopped records, and registered cleanup that fires all pending synthetic timers and finishes every child. task ci passed on the tree committed as 6da546f; mise exec go -- go test -race -count=5 -shuffle=on ./internal/app also passed. Independent verifier PASS for AC#6 and confirmed only internal/app/app_test.go changed, no tests were deleted/skipped/weakened, no timeout widening or assertion retries were added, and no protected gate files changed.
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
Replaced stop-grace test readiness polling with explicit timer and signal synchronization, and made failure cleanup release pending synthetic timers and children. Exact AC#6 count-100 race command, package stress, task ci, and independent verification passed on commit 6da546f.
