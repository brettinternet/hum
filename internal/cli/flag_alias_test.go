...
func TestFlagAliases(t *testing.T) {
	expected := map[string]map[string][]string{
		...
		"status":   {"json": {"j"}},
		"attach":   {"tail": {"n"}},
		"logs":     {"stream": {"s"}, "tail": {"n"}, "after-cursor": {"c"}, "since": nil, "limit-bytes": {"b"}, "match": {"m"}, "follow": {"f"}, "json": {"j"}},
...
func TestFlagAliasParity(t *testing.T) {
	...
	cases := []parityCase{
		...
		{"up", []string{"-d"}, []string{"--detach"}, "detach"},
		...
		{"restart", []string{"-t", "250ms"}, []string{"--timeout", "250ms"}, "timeout"},
		{"restart", []string{"--no-wait"}, []string{"--no-wait"}, "no-wait"},
