...
Only one attached run owns input; `hum logs --follow` is output-only. The owner uses raw mode and alone forwards SIGWINCH resizes. Ctrl-] detaches and leaves the child running, raw mode is restored after panic, terminal echo is child output, and Ctrl-C is forwarded while TTY input is owned; Ctrl-D and Ctrl-Z are forwarded too. SIGTERM stops the foreground incarnation and SIGHUP detaches. TTY output is merged as stdout and may contain ANSI controls. Stop/restart preserves the lease across launch cursors; remove and shutdown close it. MCP reports `tty` and provides the same bounded `input` tool.
