...
func appReadinessConfigFromProtocol(config *protocol.ReadinessConfig) *app.ReadinessConfig {
	...
	return &app.ReadinessConfig{Method: config.Method, Target: config.Target, Match: config.Match, Argv: append([]string(nil), config.Argv...), Interval: config.Interval, Timeout: config.Timeout}
}
