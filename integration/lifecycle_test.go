...
func TestSignalExitObservation(t *testing.T) {
	...
	waiter := testutil.Start(t, hum, runtime.cwd, runtime.env, "wait", "signal-observed", "--timeout", "10s", "--json")
	time.Sleep(100 * time.Millisecond)
	// Keep the child alive long enough for the pre-launch wait request to
	// subscribe before testing the signal-derived terminal result.
	started := testutil.Run(t, hum, runtime.cwd, runtime.env, "run", "signal-observed", "--detach", "--json", "--", "/bin/sh", "-c", "sleep 1; kill -TERM $$")
	...
	if err := waiter.Wait(15 * time.Second); err != nil {
...
func TestAutomaticStartup(t *testing.T) {
	...
	t.Run("attached run starts a daemon automatically", func(t *testing.T) {
		...
		if err := result.Wait(lifecycleTimeout); err != nil {
			t.Fatalf("attached automatic run: %v stdout=%q stderr=%q", err, result.Stdout(), result.Stderr())
		}
		if result.Stdout() != "attached" || result.Stderr() != "" {
			t.Fatalf("attached automatic output = stdout %q stderr %q", result.Stdout(), result.Stderr())
		}
