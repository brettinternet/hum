...
- Try `up` first; when `hum.yaml` declares `after`, it starts independent roots concurrently and launches each dependent only after every direct prerequisite is observed `ready`; each process timeout starts at its own launch or first running observation, with lexical final results. Silent services may use `ready.exec` with exact argv; probes run directly without a shell and gate startup rather than monitor liveness. The bounded CLI equivalent is `hum up --detach`; interactive plain `hum up` keeps following aggregate output after startup.
