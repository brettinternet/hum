...
- Use `wait` when a later output condition or process exit matters.
- After process-definition changes, use `restart`.
- Use `stop` only when the developer asks you to stop that process.
- Use `down` only when the developer asks you to stop everything in the project.
